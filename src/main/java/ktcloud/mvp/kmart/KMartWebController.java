...
import ktcloud.mvp.kmart.db.Product;
...
public class KMartWebController {
    ...
    public ResponseEntity<String> getProductList() {
        ...
        Map<String, Product> productMap = ProductQuery.getProductMap();

        Product cherry = productMap.get("cherry");
        Product banana = productMap.get("banana");
        Product pineapple = productMap.get("pineapple");
        Product tomato = productMap.get("tomato");
        ...
        return ResponseEntity.ok(
                "Welcome to K-Mart <hr><br>\n" +
                        ...
                        "<tr>\n" +
                        "<td><a href=\"/reqProductPayment/cherry\">체리("+cherry.getInventoryCount()+")</a><br>"+showStatus(cherry)+"</td>\n" +
                        "<td><a href=\"/reqProductPayment/banana\">바나나("+banana.getInventoryCount()+")</a><br>"+showStatus(banana)+"</td></tr>\n" +
                        "<tr>\n" +
                        "<td><a href=\"/reqProductPayment/pineapple\">파인애플("+pineapple.getInventoryCount()+")</a><br>"+showStatus(pineapple)+"</td>\n" +
                        "<td><a href=\"/reqProductPayment/tomato\">토마토("+tomato.getInventoryCount()+")</a><br>"+showStatus(tomato)+"</td></tr>\n" +
                        "</table>\n" +
                        "<br>\n" +
    ...
    private String showStatus(Product product){

        if(product.getStatus().equals("판매중")){
            return "판매중";
        } else{
            return product.getDeliveryCount()+"개 배송중";
        }
    }
    ...
    public ResponseEntity<String> reqProductPayment(@PathVariable String product) {
        ...
        return ResponseEntity.ok(createWebSocket() + "(" + product + ") 상품 결제가 완료 되었습니다 (잠시 후 배송을 시작합니다)<hr>" +
                "<p id=\"message\"></p>\n" +
